...
import { Upload, Download, Search, Grid3x3, List, FileText, Image, File, Code, X } from 'lucide-react';
...
export const FilesPage: React.FC = () => {
  ...
  const sidebarItems = [
    ...
    { icon: '📄', label: 'Files', href: '/files', active: true },
  ];
        ...
        <div className="flex gap-3 items-center">
          <div className="relative">
            ...
            <input
              ...
              className="pl-9 pr-4 py-2 glass rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
          <div className="flex gap-1 glass rounded-lg p-1">
            <button
              onClick={() => setViewMode('grid')}
              className={`p-1.5 rounded ${viewMode === 'grid' ? 'bg-blue-500 text-white' : 'text-gray-400'}`}
            >
              <Grid3x3 className="w-4 h-4" />
            </button>
            <button
              onClick={() => setViewMode('list')}
              className={`p-1.5 rounded ${viewMode === 'list' ? 'bg-blue-500 text-white' : 'text-gray-400'}`}
            >
              <List className="w-4 h-4" />
            </button>
          </div>
          <AnimatedButton variant="primary" size="sm" onClick={handleUpload}>
            ...
            >
              <GlassCard className="p-4 text-center cursor-pointer" onClick={() => setSelectedFile(file)}>
                ...
                <button
                  ...
                  className="mt-3 text-blue-500 hover:text-blue-600 transition-colors"
                >
        ...
        {selectedFile && (
          ...
          >
            ...
            >
              ...
              <AnimatedButton variant="primary" fullWidth onClick={() => handleDownload(selectedFile)}>
